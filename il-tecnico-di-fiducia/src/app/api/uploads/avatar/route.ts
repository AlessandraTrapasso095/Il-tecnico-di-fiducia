...
export async function POST(request: Request) {
  ...
  const limited = await enforceRateLimit({
    supabase,
    key: `v1:upload:avatar:user:${user.id}`,
    maxHits: 10,
    windowSeconds: 3600,
    errorMessage: "Too many avatar uploads. Please try again later.",
  });
  ...
  const previousPath = tryExtractPublicMediaPath(existing?.avatar_url ?? null);
  if (previousPath) {
    await supabase.storage.from("public-media").remove([previousPath]);
  }

  return NextResponse.json({ avatar_url: avatarUrl, path });
}
