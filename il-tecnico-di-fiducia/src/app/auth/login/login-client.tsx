...
export default function LoginClient({
  ...
  const [roleHint, setRoleHint] = useState<"customer" | "professional">(
    initialRole,
  );
      ...
      <div className="bg-surface-container-lowest rounded-[20px] p-6 shadow-[0_4px_20px_rgba(8,43,95,0.08)] border border-outline-variant/30">
        ...
        <div className="mb-6">
          <h2 className="font-headline-sm text-headline-sm text-on-surface mb-1">
            Accesso Area{" "}
            {roleHint === "professional" ? "Professionista" : "Cliente"}
          </h2>
        ...
        <form className="space-y-4" onSubmit={onSubmit}>
          ...
          <button
            className="inline-flex w-full items-center justify-center gap-2 bg-[#FF8500] hover:bg-[#FF9A2B] text-white font-button text-button py-4 rounded-full transition-all duration-200 active:scale-[0.99] shadow-lg shadow-orange-500/20 disabled:cursor-wait disabled:opacity-60"
            type="submit"
            disabled={loading}
          >
            {loading ? (
