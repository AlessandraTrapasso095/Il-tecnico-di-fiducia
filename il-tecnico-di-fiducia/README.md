...
- `MAIL_FROM_EMAIL=info@iltecnicodifiducia.it`
- `MAIL_FROM_NAME=Il Tecnico di Fiducia`
- `SUPPORT_ADMIN_EMAIL=admin@iltecnicodifiducia.it`

If `RESEND_API_KEY` is missing, the app still saves support tickets and admin
replies, but logs that email delivery was skipped. Supabase Auth emails remain
controlled by the Supabase custom SMTP settings above.
