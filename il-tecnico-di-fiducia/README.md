...
use a server-only email utility. Configure these values in `.env.local`:

- `RESEND_API_KEY`: API key for Resend transactional email delivery.
- `EMAIL_FROM="Il Tecnico di Fiducia <info@iltecnicodifiducia.it>"`
- `SUPPORT_ADMIN_EMAIL=admin@iltecnicodifiducia.it`

Before testing delivery, verify the sender/domain `iltecnicodifiducia.it` in
Resend and restart `npm run dev` after editing `.env.local`. If
`RESEND_API_KEY` is missing, the app still saves support tickets and replies,
but logs that email delivery was skipped. If Resend rejects a message, the API
logs the provider error server-side and keeps the ticket/message saved.
Supabase Auth emails remain controlled by the Supabase custom SMTP settings
above.
