...
- Gmail SMTP with an app password, Postmark, AWS SES, SendGrid, Brevo, or
...
use a server-only Nodemailer SMTP utility. Configure these values in
`.env.local`:

- `SMTP_HOST=smtp.gmail.com`
- `SMTP_PORT=465`
- `SMTP_SECURE=true`
- `SMTP_USER=ricettetaglisicuri@gmail.com`
- `SMTP_PASS=PASSWORD_APP_GMAIL`
- `EMAIL_FROM="Il Tecnico di Fiducia <ricettetaglisicuri@gmail.com>"`
- `SUPPORT_ADMIN_EMAIL=admin@iltecnicodifiducia.it`

Use a Gmail app password, not the normal Gmail account password. Restart
`npm run dev` after editing `.env.local`. If `SMTP_USER` or `SMTP_PASS` is
missing, the app still saves support tickets and replies, but logs that email
delivery was skipped. If SMTP rejects a message, the API logs the provider
error server-side and keeps the ticket/message saved. Supabase Auth emails
remain controlled by the Supabase custom SMTP settings above.
